...
def load_professional_styles():
    ...
    /* Hide only footer and header, keep MainMenu visible */
    footer, header {visibility: hidden;}
    .stDeployButton {visibility: hidden;}
    
    /* Style the hamburger menu properly */
    #MainMenu {
...
def create_professional_kpi_card(title: str, value: str, unit: str, 
    ...
    """Create simple KPI cards using only Streamlit native components"""
    ...
    # Create simple but clean KPI display
    st.markdown(f"### {icon} {title}")
    st.markdown("*Real-time Analytics*")
    
    # Main metric using Streamlit's metric component
    delta_color = "normal" if status == "success" else "inverse" if status == "danger" else "off"
    st.metric(
        label="Current Value",
        value=f"{value} {unit}",
        delta=trend_text,
...
def create_excel_export(data_dict: Dict) -> bytes:
    ...
    try:
        ...
        # If openpyxl is not available, create a simple CSV export instead
        output = io.StringIO()
        if "simulation_data" in data_dict:
            data_dict["simulation_data"].to_csv(output, index=False)
